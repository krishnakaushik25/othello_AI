...
class Othello:
    ...
    def move_generator(self, player):
        ...
        for index, direction in itertools.product(pieces_indices, DIRECTIONS):
    ...
    def single_piece_move_generator(self, player, opponent, start, direction, moves):
        ...
        while not self.out_of_bounds(x, y):
            if self.state[x, y] == opponent:
                ...
                if last_seen_opponent_piece is not None and not self.out_of_bounds(x, y):
    ...
    def simple_evaluation_fn(self):
        ...
        if game_status in [GAME_IN_PROGRESS, DRAW]:
            ...
            self.last_move.value = 100 if MAXIMIZING_PLAYER == BLACK else -100
        elif game_status == WHITE_WON:
            self.last_move.value = 100 if MAXIMIZING_PLAYER == WHITE else -100
